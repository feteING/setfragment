...
public final class FragmentStack {
    ...
    /**
     * @param isResume true 回退刷新界面
     * @return
     */
    public Fragment pop(boolean isResume) {
        if (mStack.size() > 1) {
            FragmentTransaction transaction = mFragmentManager.beginTransaction();
            transaction
            ...
            popShowFragment(transaction, f, f.getTag(), isResume);
            transaction.commit();
            return popFragment;
        }
        return null;
    ...
    private void popShowFragment(FragmentTransaction transaction, Fragment fragment, String tag, boolean isResume) {
        if (fragment == null) {
            return;
        }
        if (fragment.isAdded()) {
